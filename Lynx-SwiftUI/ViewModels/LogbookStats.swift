...
@Observable final class LogbookStats {
  ...
  func getConfiguredLogbookData(at index: Int) -> ConfiguredLogbookData? {
    ...
    var capitalizedConditions = logbook.conditions.map {
      $0.trimmingCharacters(in: .whitespacesAndNewlines).replacingOccurrences(of: "_", with: " ")
        .capitalized
    }
    ...
    return ConfiguredLogbookData(
      ...
      topSpeed: String(
        format: "%.1f\(profileManager.measurementSystem.milesOrKilometersPerHour)", logbook.topSpeed
      )
    )
  }
  ...
  func rangeDataPerSession<T>(propertyExtractor: (Logbook.Detail) -> T) -> [(
    date: Date, min: Double, max: Double
  )] {
    logbooks.rangeDataPerSession(propertyExtractor: propertyExtractor)
  }

  func maxAndMinOfDateAndValues(fromRangeData data: [(date: Date, min: Double, max: Double)]) -> (
    earliest: Date, latest: Date, smallest: Double, largest: Double
  ) {
...
extension Logbooks {
  ...
  func lifetimeAverages(measurementSystem: MeasurementSystem) -> [[Stat]] {
    ...
    return [
      [
        Stat(
          label: "run vertical", information: averageVerticalFeet, systemImageName: "arrow.down"),
  ...
  func lifetimeBest(measurementSystem: MeasurementSystem) -> [[Stat]] {
    // Top speed
    let topSpeed = String(
      format: "%.1f \(measurementSystem.milesOrKilometersPerHour)",
      self.map { $0.topSpeed }.max() ?? 0.0)

    // Tallest run
    let tallestRun = String(
      format: "%.1f \(measurementSystem.feetOrMeters)",
      self.map { $0.verticalDistance }.max() ?? 0.0)
