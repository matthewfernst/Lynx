...
@Observable final class PartyHandler {
  ...
  func fetchPartyDetails(
    partyId: String, sortBy: LeaderboardSort = .verticalDistance, timeframe: Timeframe = .season,
    resort: String? = nil, completion: ((Bool) -> Void)? = nil
  ) {
  ...
  func editParty(
    partyId: String, name: String, description: String?, completion: @escaping (Bool) -> Void
  ) {
    isEditingParty = true
    errorMessage = nil
  ...
  func inviteUserToParty(
    partyId: String, userId: String, completion: @escaping (Result<Void, Error>) -> Void
  ) {
    isInvitingUser = true
    errorMessage = nil
