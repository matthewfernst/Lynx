...
@Observable final class FolderConnectionHandler: ObservableObject {
  ...
  func picker(didPickDocumentsAt url: URL, dismissForUpload: Binding<Bool>) {
    ...
    NSFileCoordinator().coordinate(readingItemAt: url, error: &error) { (url) in
      if url.lastPathComponent == "GPSLogs" {
        ...
        Logger.folderConnectionHandler.error(
          "Selected URL is not the correct directory: \(url.path)")
    ...
    guard
      let directoryEnumerator = fileManager.enumerator(
        ...
        errorHandler: { (url, error) -> Bool in
          ...
          Logger.folderConnectionHandler.error(
            "Failed to access file at URL: \(url), error: \(error.localizedDescription)")
  ...
  private func putSlopeFiles(
    urlEndPoint: String, slopeFileURL: URL, completion: @escaping (Result<Int, Error>) -> Void
  ) {
    ...
    let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
      ...
      if let response = response as? HTTPURLResponse {
        if response.statusCode == 200 {
          ...
          let error = NSError(
            domain: "Status code is not 200", code: response.statusCode, userInfo: nil)
          completion(.failure(error))
        }
      }
    }
  ...
  private func isSlopesFiles(_ fileURL: URL) -> Bool {
    return !fileURL.hasDirectoryPath && fileURL.path.lowercased().contains("gpslogs")
      && fileURL.pathExtension == "slopes"
  }
  ...
  func getNonUploadedSlopeFiles(forURL url: URL, completion: @escaping ([URL]?) -> Void) {
    ...
    ApolloLynxClient.getUploadedLogs { [unowned self] result in
      ...
      case .failure(let error):
        Logger.folderConnectionHandler.error(
          "Error in trying to find non uploaded slope files: \(error.localizedDescription)")
        completion(nil)
      }
    }
  }
  ...
  func uploadNewFiles(_ nonUploadedSlopeFiles: [URL], completion: (() -> Void)?) {
    ...
    do {
      let contents = try FileManager.default.contentsOfDirectory(
        at: gpsLogsURL, includingPropertiesForKeys: nil)
      if contents.allSatisfy({ $0.pathExtension == "slopes" }) {
        ...
        ApolloLynxClient.createUserRecordUploadUrl(filesToUpload: requestedPathsForUpload) {
          [unowned self] result in
          switch result {
          case .success(let urlsForUpload):
            guard gpsLogsURL.startAccessingSecurityScopedResource() else {
              Logger.folderConnectionHandler.error(
                "File Access Error in uploadNewFiles() createUser")
              self.showFileAccessError()
              return
            }
            ...
            self.uploadTimer = Timer.scheduledTimer(withTimeInterval: 1.5, repeats: true) { timer in
              ...
              Logger.folderConnectionHandler.info(
                "Uploading newly found file: \(fileURL.lastPathComponent)")

              self.putSlopeFiles(
                urlEndPoint: urlsForUpload[currentFileNumberBeingUploaded], slopeFileURL: fileURL
              ) { result in
                switch result {
                case .success(_):
                  ...
                  DispatchQueue.main.async {
                    withAnimation {
                      self.uploadProgress =
                        Double(currentFileNumberBeingUploaded) / Double(totalNumberOfFiles)
                      let fileURLString = fileURL.lastPathComponent.replacingOccurrences(
                        of: "%", with: " ")
                      if let startRange = fileURLString.range(of: "-"),
                        let endRange = fileURLString.range(of: ".slopes")
                      {
                        self.currentFileBeingUploaded = String(
                          fileURLString[startRange.upperBound..<endRange.lowerBound]
                            .trimmingCharacters(in: .whitespaces))
                        Logger.folderConnectionHandler.debug(
                          "Current file being uploaded: \(self.currentFileBeingUploaded)")
                      }
                    }
                ...
                case .failure(let error):
                  Logger.folderConnectionHandler.error(
                    "Failed to upload \(fileURL.lastPathComponent): \(error.localizedDescription)")
                }
              }
            }
      ...
      Logger.folderConnectionHandler.error(
        "File Access Error in uploadNewFiles(), Error: \(error.localizedDescription)")
      showFileAccessError()
      return
    }
