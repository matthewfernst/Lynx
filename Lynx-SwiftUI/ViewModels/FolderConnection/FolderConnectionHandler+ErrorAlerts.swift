...
extension FolderConnectionHandler {
  ...
  func showFileAccessError() {
    errorAlert = Alert(
      title: Text("File Permission Error"),
      message: Text(
        "This app does not have permission to your files on your iPhone. Please allow this app to access your files by going to Settings."
      ),
  ...
  func showFileExtensionNotSupported(extensions: [String]) {
    errorAlert = Alert(
      ...
      message: Text(
        "Only '.slope' file extensions are supported, but recieved \(extensions.joined(separator: ", ")) extensions. Please try again."
      )
    )
  }
  ...
  func showWrongDirectorySelected(directory: String) {
    errorAlert = Alert(
      ...
      message: Text(
        "The correct directory for uploading 'Slopes', but recieved '\(directory)'. Please try again."
      )
    )
  }
}
