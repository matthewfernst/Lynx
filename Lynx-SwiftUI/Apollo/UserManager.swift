  ...
  var lynxToken: ExpirableLynxToken? {
    get {
      do {
        ...
        Logger.userManager.error(
          "KeychainManager failed to handle getting the ExpirableLynxToken. Please check the logs.")
        cleanUpFailedReAuth()
        return nil
      }
    }
    set {  // if we do UserManager.shared.lynxToken = nil -> we want to delete the lynxToken
      do {
        ...
        Logger.userManager.error(
          "KeychainManager failed to handle setting the ExpirableLynxToken. Please check the logs.")
        cleanUpFailedReAuth()
      }
    }
  }
