...
struct LeaderAttributes: Identifiable, Hashable {
  ...
  init(leader: Leaderboard, category: LeaderboardSort, profilePictureURL: URL?) {
    ...
    self.stat = LeaderAttributes.getNumericStatFrom(
      leaderStat: leader.stat, specificStatSortedBy: category)
  }

  private static func getNumericStatFrom(
    leaderStat: LeaderStat?, specificStatSortedBy: LeaderboardSort
  ) -> Double {
  ...
  static func formattedStatLabel(
    ...
    case .distance, .verticalDistance:
      return String(
        format: "%.1fk \(measurementSystem.feetOrMeters)", stat / Constants.thousandDivisor)
    case .runCount:
      return "\(Int(stat))"
    case .topSpeed:
