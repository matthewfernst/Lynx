...
struct EditProfileView: View {
  ...
  var body: some View {
    NavigationStack {
      ...
      .confirmationDialog(
        "Confirm Deletion of Account", isPresented: $showDeleteAccountConfirmation
      ) {
      ...
      .alert("Feature Not Available", isPresented: $showMergeAccountsNotAvailable) {
      } message: {
        Text(
          "Merging your Google or Apple accounts together is not yet available. Stay tuned for updates!"
        )
      }
      .toolbar {
    ...
    profileManager.edit(
      ...
      ApolloLynxClient.editUser(profileChanges: profileChanges) { result in
        ...
        case .success(let newProfilePictureURL):
          Logger.editProfileHandler.info(
            "Retrieved new profile picture URL: \(newProfilePictureURL)")
        ...
        case .failure(_):
          Logger.editProfileHandler.error(
            "Failed to get new profile picture URL. Using old picture.")
      ...
      ApolloLynxClient.createUserProfilePictureUploadUrl { result in
        ...
        case .success(let url):
          ...
          guard let url = URL(string: url) else {
            Logger.editProfileHandler.error(
              "Failed to convert String URL into URL coming back from createUserProfilePictureUploadURL"
            )
            return
          }
          ...
          URLSession.shared.reset { [request] in
            URLSession.shared.dataTask(with: request) { data, response, error in
              if let error = error {
                Logger.editProfileHandler.error(
                  "Failed to upload new profile picture to S3: \(error)")
                return
              }
...
extension EditProfileView {
  private final class TOCropDelegate: NSObject, TOCropViewControllerDelegate {
    ...
    func cropViewController(
      _ cropViewController: TOCropViewController, didCropToCircularImage image: UIImage,
