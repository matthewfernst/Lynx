...
struct EditPartyView: View {
  ...
  init(
    partyHandler: PartyHandler, partyId: String, currentName: String, currentDescription: String?
  ) {
  ...
  private func editParty() {
    ...
    partyHandler.editParty(partyId: partyId, name: partyName, description: descriptionToSave) {
      success in
      if success {
        dismiss()
      }
