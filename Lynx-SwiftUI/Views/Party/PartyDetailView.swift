...
struct PartyInvitedUsersListSection: View {
  ...
  var body: some View {
    ...
    .alert("Revoke Invitation", isPresented: $showRevokeConfirmation) {
      ...
      if let user = inviteToRevoke {
        Text(
          "Are you sure you want to revoke the invitation for \(user.firstName) \(user.lastName)?")
      }
    }
  }
}
...
struct PartySettingsView: View {
  ...
  init(
    partyHandler: PartyHandler, partyId: String, partyName: String, partyDescription: String?,
    invitedUsers: [PartyUser], isManager: Bool, onDismiss: @escaping () -> Void
  ) {
