    ...
    var body: some View {
        List {
            ...
            Section("Session Stats") {
                ...
                DetailRow(label: "Vertical", value: String(format: "%.0f \(profileManager.measurementSystem.feetOrMeters)", logbook.verticalDistance))
                DetailRow(label: "Top Speed", value: String(format: "%.1f \(profileManager.measurementSystem.milesOrKilometersPerHour)", logbook.topSpeed))
            }
                ...
                Section("Individual Runs (\(logbook.details.count))") {
                    ForEach(Array(logbook.details.enumerated()), id: \.offset) { index, detail in
                        VStack(alignment: .leading, spacing: 8) {
                            HStack {
                                Image(systemName: detail.type.rawValue == "LIFT" ? "arrow.up.circle.fill" : "arrow.down.circle.fill")
                            ...
                            Grid(alignment: .leading, horizontalSpacing: 16, verticalSpacing: 8) {
                                GridRow {
                                    Label("\(String(format: "%.1f", detail.topSpeed)) \(profileManager.measurementSystem.milesOrKilometersPerHour)", systemImage: "speedometer")
                                        .font(.caption)
                                    Label(formatDistance(detail.distance), systemImage: "arrow.left.and.right")
                                        .font(.caption)
                                }
                                GridRow {
                                    Label("\(String(format: "%.0f", detail.verticalDistance)) \(profileManager.measurementSystem.feetOrMeters)", systemImage: "arrow.up.and.down")
                                        .font(.caption)
