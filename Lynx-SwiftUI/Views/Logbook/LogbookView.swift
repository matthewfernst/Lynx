...
struct LogbookView: View {
      ...
      NavigationStack {
        scrollableSessionSummaries
          ...
          .alert("Unable to Load Logs", isPresented: $showLoadError) {
            ...
            Text(
              "We couldn't load your ski logs. Please check your internet connection and try again."
            )
          }
  ...
  private var conditionsChart: some View {
    ...
    VStack(spacing: 0) {
      HStack(spacing: 20) {
        ...
        .animation(
          .spring(response: 0.6, dampingFraction: 0.8), value: sortedConditions.map(\.count)
        )
        .onAppear {
  ...
  private var logsBySeasonGrouped:
    [(season: String, logs: [(index: Int, data: ConfiguredLogbookData)])]
  {
    ...
    for (index, logbook) in logbookStats.logbooks.enumerated() {
      guard let date = dateFormatter.date(from: logbook.startDate),
        let configuredData = logbookStats.getConfiguredLogbookData(at: index)
      else {
        continue
      }
    ...
    if let url = BookmarkManager.shared.bookmark?.url {
      folderConnectionHandler.getNonUploadedSlopeFiles(forURL: url) { files in
        if let files, !files.isEmpty {
          ...
          let isFirstTimeSetup =
            UserDefaults.standard.bool(forKey: "hasCompletedInitialUpload") == false
