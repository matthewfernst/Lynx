...
const oauthSignIn = async (
  ...
  return await oauthLogin(
    idKeyFromIdType[OAuthType[type]],
    id,
    args.email,
    args.userData,
  );
...
const isValidFacebookToken = async (
  id: string,
  token: string,
): Promise<boolean> => {
  ...
  const { data: facebookData }: { data: FacebookData } =
...
const oauthLogin = async (
  ...
  if (!user) {
    if (!email || !userData) {
      const errorMessage =
        "Must Provide Email And UserData On Account Creation";
      console.error(
        `${errorMessage}. Provided email: ${email}, userData: ${JSON.stringify(userData)}`,
      );
...
const createNewUser = async (
  ...
  await putItem(USERS_TABLE, {
    ...
    ...Object.assign(
      {},
      ...userData.map((item) => ({ [item.key]: item.value })),
    ),
  });
  return userId;
};
